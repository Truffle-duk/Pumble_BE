...
export const selectPostList = "SELECT p.post_id, p.title, p.content, p.created_at, COUNT(c.comment_id) as comment_count FROM Post as p LEFT JOIN Comment as c ON p.post_id = c.post_id WHERE p.group_id = ? and p.type = 'normal' GROUP BY p.post_id, p.title, p.content, p.created_at ORDER BY p.created_at LIMIT ?, 10;";
export const selectNoticeList = "SELECT p.post_id, p.title, p.created_at, g.nickname, g.group_user_id FROM Post as p JOIN GroupUser as g ON p.group_user_id = g.group_user_id WHERE p.group_id = ? and p.type = 'notice' ORDER BY p.created_at LIMIT ?, 10;";
